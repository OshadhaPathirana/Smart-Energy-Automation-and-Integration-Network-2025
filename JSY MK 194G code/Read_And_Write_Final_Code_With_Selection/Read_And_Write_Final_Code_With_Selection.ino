...
void manualInputAndSend() {
  static float v_1 = 220.0, i_1 = 5.0, f_1 = 50.0;  // Default values
  unsigned long lastSendTime = 0;  // Track last send time
  const unsigned long sendInterval = 2000;  // Send data every 2 seconds

  Serial.println("Enter Voltage, Current, and Frequency (e.g., 220.5 4.8 49.9):");

  while (true) {
    // Check for Serial input and update values if available
    if (Serial.available() > 0) {
      String input = Serial.readStringUntil('\n');  
      input.trim();

      if (sscanf(input.c_str(), "%f %f %f", &v_1, &i_1, &f_1) == 3) {
        Serial.println("Values updated!");
      } else {
        Serial.println("Invalid input! Use format: Voltage Current Frequency");
      }
    }
    ...
    if (millis() - lastSendTime >= sendInterval) {
      ...
      int v1 = v_1 * 1000;  // Convert to int for InfluxDB
      int f1 = f_1 * 1000;
      int i1 = i_1 * 1000;
