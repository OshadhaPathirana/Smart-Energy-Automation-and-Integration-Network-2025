...
def write_voltage_to_influx(predicted_voltage):
    predicted_data = {
        "point1": {"Inverter_ID": "1", "Measurement": "voltage", "Value": predicted_voltage},
        "point2": {"Inverter_ID": "2", "Measurement": "voltage", "Value": predicted_voltage},
        "point3": {"Inverter_ID": "3", "Measurement": "voltage", "Value": predicted_voltage},
        "point4": {"Inverter_ID": "4", "Measurement": "voltage", "Value": predicted_voltage}
    }
...
def simulate_inverter(initial_voltage, target_voltage, Kp, Ki, Kd):
    ...
    while abs(target_voltage - current_voltage_1) > 1 or abs(target_voltage - current_voltage_2) > 1:
        ...
        predicted_voltage = int((current_voltage_1 + current_voltage_2) / 2)
...
def simulate_inverter_increasing(initial_voltage, target_voltage, Kp, Ki, Kd):
    ...
    while abs(target_voltage - current_voltage) > 0.1:
        ...
        predicted_voltage = int(current_voltage)
        write_voltage_to_influx(predicted_voltage)
...
while True:
    ...
    val = simulate_inverter(initial_voltage, target_voltage, Kp=80, Ki=1.1, Kd=1)
